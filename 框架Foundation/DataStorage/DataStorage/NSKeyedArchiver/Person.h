...
/**
 ...
 当我们需要将模型对象编码到文档中，需要时再对其解码读取时，在 runtime 使用 NSCoder 对象进行编码与解码操作。
 但 NSCoder 本身是一个抽象类。Apple 建议通过 NSCoder 的具体子类 NSKeyedArchiver 和 NSKeyedUnarchiver，也就是使用基于键的归档技术。
 被编码与解码的对象必须遵守 <NSCoding> 协议并实现以下方法：
