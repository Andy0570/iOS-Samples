    ...
    switch (indexPath.section) {
        case 0: {
            switch (indexPath.row) {
                ...
                case 2: {
                    // 标准的 Alert 样式：取消+删除
                    [self createCancelAndDestructiveAlert];
                    break;
                }
                case 3: {
                    // 带有多个按钮的 Alert 样式
...
#pragma mark 标准的 Alert 样式：取消+确定
...
#pragma mark 标准的 Alert 样式：取消+删除
- (void) createCancelAndDestructiveAlert {
    //  1.实例化UIAlertController对象
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"标准的Alert 样式" message:@"UIAlertControllerStyleAlert" preferredStyle:UIAlertControllerStyleAlert];
