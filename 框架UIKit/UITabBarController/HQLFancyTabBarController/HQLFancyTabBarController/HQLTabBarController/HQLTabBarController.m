...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat tabBarWidth = CGRectGetWidth(self.tabBar.frame) ? : 217.0f;
    CGFloat tabBarHeight = CGRectGetHeight(self.tabBar.frame) ? : 76.0f;
        
    // 如果 tabBar 不隐藏
    if (!self.tabBarHidden) {
        if (@available(iOS 11.0, *)) {
            CGFloat safeAreaBottom = [[UIApplication sharedApplication] keyWindow].safeAreaInsets.bottom;
            NSLog(@"safeAreaBottom = %f", safeAreaBottom);
    ...
    self.tabBar.frame = CGRectMake(roundf((viewSize.width - tabBarWidth) / 2), tabBarStartingY, tabBarWidth, tabBarHeight);
...
#pragma mark - UIViewController+HQLFancyTabBarController
...
@implementation UIViewController (HQLTabBarController)
