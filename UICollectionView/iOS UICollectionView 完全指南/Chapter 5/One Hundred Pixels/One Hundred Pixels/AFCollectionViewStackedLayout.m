...
@property (nonatomic, strong) AFCollectionViewFlowLayout *gridLayout;
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)path
{
    ...
    AFCollectionViewLayoutAttributes* attributes = [AFCollectionViewLayoutAttributes layoutAttributesForCellWithIndexPath:path];
...
-(NSArray*)layoutAttributesForElementsInRect:(CGRect)rect
{
    ...
    for (int stack = 0; stack < self.numberOfStacks; stack++)
    {
        ...
        if (CGRectIntersectsRect(stackFrame, rect))
        {
            ...
            [attributes addObject:[self layoutAttributesForSupplementaryViewOfKind:[AFCollectionViewHeaderView kind] atIndexPath:[NSIndexPath indexPathForItem:0 inSection:stack]]];
        }
    }
...
- (void)setPinchedStackIndex:(NSInteger)pinchedStackIndex
{
    ...
    {
        if (self.isPinching)
            self.gridLayout = [[AFCollectionViewFlowLayout alloc] init];
        else
            self.gridLayout = nil;
    }
    [self invalidateLayout];
