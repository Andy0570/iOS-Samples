...
@interface KCLFileManager ()

@property (strong, nonatomic) NSFileManager *manager;

@end

@implementation KCLFileManager
...
+ (BOOL)isEmptyItemAtPath:(NSString *)path error:(NSError *__autoreleasing *)error {
    return ([self isFileAtPath:path error:error] && [[self sizeOfItemAtPath:path error:error] intValue] == 0) ||
           ([self isDirectoryAtPath:path error:error] && [[self listFilesInDirectoryAtPath:path deep:NO] count] == 0);
}
