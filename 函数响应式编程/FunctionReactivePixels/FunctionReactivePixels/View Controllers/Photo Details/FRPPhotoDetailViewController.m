...
- (void)viewDidLoad {
    ...
    // Configure self
    self.title = self.viewModel.photoNamel;
    ...
    ratingLabel.font = [UIFont systemFontOfSize:80];
    ...
    RAC(photoNameLabel, text) = RACObserve(self.viewModel, photoNamel);
    ...
    voteButton.rac_command = [[RACCommand alloc] initWithEnabled:self.viewModel.voteCommand.enabled signalBlock:^RACSignal *(id input) {
        ...
        if ([[PXRequest apiHelper] authMode] == PXAPIHelperModeNoAuth) {
            ...
            authSignal = [[RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
                ...
                FRPLoginViewController *viewController = [[FRPLoginViewController alloc] initWithNibName:NSStringFromClass([FRPLoginViewController class]) bundle:nil];
