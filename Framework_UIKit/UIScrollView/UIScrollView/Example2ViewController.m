...
@interface Example2ViewController () <UIScrollViewDelegate>
...
- (void)handleDoubleTap:(UITapGestureRecognizer *)doubleTap {
    if (self.scrollView.zoomScale > self.scrollView.minimumZoomScale) {
        ...
        */
        ...
        // 5.将要显示的 imageView 区域显示到 scrollView
...
- (CGRect)zoomRectForScrollView:(UIScrollView *)scrollView withScale:(CGFloat)scale withCenter:(CGPoint)center {
    ...
    // 4.点击位置x坐标减去1/2图像宽度，可以得到要显示imageView的原点 x 坐标 y 坐标类似
...
- (void)scrollViewDidZoom:(UIScrollView *)scrollView {
    // 计算 imageView 缩小到最小时，imageView 与屏幕边缘距离
