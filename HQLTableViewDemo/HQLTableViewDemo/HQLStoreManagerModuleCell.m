...
// 32+68
const CGFloat HQLStoreManagerModuleHeight = 100.0f;
static NSString * const KPlistFileName = @"mineStoreManagerModule.plist";
...
@interface HQLStoreManagerModuleCell () <UICollectionViewDataSource, UICollectionViewDelegate>
@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, copy) NSArray<HQLTableViewModel *> *dataSourceArray;
@property (nonatomic, assign) BOOL delegateFlag;
@end

@implementation HQLStoreManagerModuleCell
...
- (void)setDelegate:(id<HQLStoreManagerModuleDelegate>)delegate {
    _delegate = delegate;
    _delegateFlag = [delegate respondsToSelector:@selector(selectStoreManagerItemAtIndex:)];
}
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
        reuseableView = [self configureCollectionName:@"店长特权" atIndexPath:indexPath];
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (_delegateFlag) {
        [self.delegate selectStoreManagerItemAtIndex:indexPath.item];
    }
}
