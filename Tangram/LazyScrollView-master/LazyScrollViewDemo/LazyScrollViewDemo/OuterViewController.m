...
/**
 MARK: LazyScrollView 可以用作另一个 ScrollView 的子视图。
 
 例如: 您可以将 LazyScrollView 设置为 UITableView 的 footerView。
 那么 outerScrollView 应该是 TableView。
 必须在 outerScrollView 的删除之前将此属性设置为 nil。
 */
...
{
    if (self = [super init]) {
        // 将 LazyScroll 作为另一个 ScrollView 的子视图
        self.title = @"Outer 外层";
    }
    return self;
}
