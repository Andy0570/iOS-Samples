...
消息和方法之间的区别：**方法是指一块可以执行的代码，而消息是指要求类或对象执行某个方法的动作。**
...
如果应用向某个对象发送了其无法响应的消息，那么程序就会抛出异常（exception）。异常也称为**运行时错误**（run-time error）。

未知选择器：某个对象收到了其没有实现的消息。
